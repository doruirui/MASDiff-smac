...
对同一个状态 `(OD, tau)`，从策略分布中采样 `S` 个候选路网权重：
...
其中，`F_psi` 表示扩散奖励模型，`R_s` 表示候选路网权重 `r_s` 在当前状态下的评价分数。

### 步骤 5：计算 baseline

计算当前 batch 内的平均奖励作为 baseline：
...
重复上述过程，直到训练轮数达到设定值，或者策略模型生成的路网权重在扩散模型评价下趋于稳定。
...
在该训练过程中，SUMO 场景被建模为一个策略优化问题。策略模型以 `OD` 和当前路网排队长度状态 `tau` 作为输入，输出路网权重 `r` 的概率分布。随后，从该分布中采样多个候选路网权重，并利用扩散奖励模型 `F_psi` 对每个候选方案进行评分。

REINFORCE 算法根据奖励结果更新策略模型：高于平均奖励的候选权重会被增强，低于平均奖励的候选权重会被抑制。经过多轮训练后，策略模型能够在给定 `OD` 和排队长度状态 `tau` 的条件下，生成更可能获得高奖励的路网权重。
